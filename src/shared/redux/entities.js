...
const BATCH_DELETE_ENTITIES = "contenthouse/entity/BATCH_DELETE_ENTITIES";
...
export function batchDeleteEntities(payload) {
  return { type: BATCH_DELETE_ENTITIES, payload: payload };
}
...
function mergeEntities(entityKeys, currentState, action, options = {}) {
  ...
  entityKeys.forEach((entityKey) => {
    ...
    const updatedActionEntityState = removeStaleEntities(
      currentEntityState,
      actionEntityState
    );

    updatedState[entityKey] = deepmerge(
      currentEntityState,
      updatedActionEntityState,
      mergeOptions
    );
  });
...
export function digObject(target, keyString, defaultValue) {
  ...
  const value = keys.reduce(
    (acc, key) => {
      const accValue =
        typeof acc === "undefined" || acc === null ? acc : acc[key];
      return accValue;
    },
    { ...target }
  );
...
function batchRemoveEntities(state, action) {
  const newState = cloneDeep(state);
  var result = Object.keys(state.companies).map((key) => [Number(key), state.companies[key]]);
  for (let p of result) {
    for (let i of action.payload) {
      if(i === p[1].name){
        delete newState.companies[p[1].id];
      }
    }
  }
  return newState;
...
export default function reducer(state = initialState, action = {}) {
  switch (action.type) {
    ...
    case BATCH_DELETE_ENTITIES:
      return batchRemoveEntities(state, action);
    default:
      return state;
  }
}
